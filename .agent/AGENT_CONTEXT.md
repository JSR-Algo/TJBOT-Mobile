...
- BLE wire protocol message shapes → tbot-firmware (sys-18)
- AI safety filters and prompts → tbot-ai-services (sys-05)
- Infrastructure, IAM, KMS, CI/CD → tbot-infra (sys-13)
- Root TBOT docs at `/Users/manhhodinh/Documents/TBOT/docs/` → separate git repo
...
1. Edit any file outside `/Users/manhhodinh/Documents/TBOT/tbot-mobile/`
   (exceptions: root CLAUDE.md, root AGENTS.md — but only Step 8/PR8 cleanup,
   and only with explicit user instruction).
2. Modify `docs/site/api/openapi.json` — that lives in the root TBOT docs repo.
   tbot-mobile only reads `migrate-ui-ux-to-mobile-app-docs/api/openapi.json`
   (symlink). Do not write through the symlink.
3. Change BLE message schemas (`src/services/ble/`) without escalating to
   tbot-firmware first. Wire protocol is sys-18 — cross-repo boundary.
...
tbot-mobile consumes (read-only):
- `migrate-ui-ux-to-mobile-app-docs/api/openapi.json`
  → symlink to `/Users/manhhodinh/Documents/TBOT/docs/site/api/openapi.json`
  (root TBOT docs repo; tbot-mobile is a consumer only)
...
tbot-mobile emits (written by generators, not hand-edited):
...
| Safety filters (sys-05) | tbot-ai-services — red-team suite must remain 100% | `docs/site/safety/*.md` |
| Auth / COPPA (sys-01) | tbot-backend — legal review may be required | `docs/site/legal/coppa-*.md` |
| Infra / IAM / KMS (sys-13) | tbot-infra — staging validation required | `docs/runbooks/staging-*.md` |
| Realtime session (sys-04) | tbot-backend + tbot-ai-services | `docs/site/architecture/realtime-overview.md` |
| Mobile UX (sys-16) | tbot-mobile (this repo) | `docs/site/software/systems/16-*.md` |
...
## Multi-agent lane ownership (within tbot-mobile PRs)
