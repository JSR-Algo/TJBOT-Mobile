# T5.4 Parent-session assignment bridge: H1 Pass 54 handoff
...
`confirmAssignmentId=PASS54`, accepts no identity override, performs one HTTP `POST`, requires
...
assignment. Pass 54 adds a bounded active-root acquisition wait of at most five seconds. Both
...
HTTP, result, or helper-gate failure freezes Pass 54. After the assignment test is invoked once,
...
PASS54=/Users/manhhodinh/Documents/TBOT/robot/docs/evidence/t54-live-20260820-final-closeout/final-parent-sla-pass-54
SIGNERS="$PASS54/parent-session-bridge/signers"
...
PASS54=/Users/manhhodinh/Documents/TBOT/robot/docs/evidence/t54-live-20260820-final-closeout/final-parent-sla-pass-54
BRIDGE="$PASS54/parent-session-bridge"
PROBE_LOG="$BRIDGE/session-probe.log"
COLLECTOR="$PASS54/parent-progress"
...
JWT-shaped output freezes Pass 54 before assignment. The probe may spend at most five seconds
...
## Pass 54 one-shot pre-POST gate

The following conditions must already be true in the same immutable Pass 54 evidence window:
...
5. No assignment has been created in Pass 54 and the one-shot command below has never been
   entered.
...
PASS54=/Users/manhhodinh/Documents/TBOT/robot/docs/evidence/t54-live-20260820-final-closeout/final-parent-sla-pass-54
TIMELINE="$PASS54/live-capture/timeline.log"
COLLECTOR="$PASS54/parent-progress"
COLLISION_LOG="$PASS54/collector-logcat.txt"
: "${T54_PASS54_CAPTURE_PID:?set to the already-running Pass 54 capture PID}"

test "$T54_PASS54_CAPTURE_PID" -gt 1
kill -0 "$T54_PASS54_CAPTURE_PID"
test -s "$TIMELINE"
...
path is fixed by the Java bridge. Do not change `PASS54`, `resultPath`, the class name, or the
arming value.
...
HOST_RESULT="$BRIDGE/t54-pass54-assignment.json"
DEVICE_RESULT=/storage/emulated/0/Android/data/com.TJBotmobile/files/t54-pass54-assignment.json
...
T54_COLLECTOR_OUT="$PASS54/parent-progress" \
T54_COLLECTOR_MODE=with-uiautomation-lease \
T54_COLLECTOR_UIAUTOMATOR_COLLISIONS=0 \
  bash /Users/manhhodinh/Documents/TBOT/t54-parent-collector-pass9.sh -- \
  bash -c 'mkdir "$1"; shift; exec "$@"' _ "$BRIDGE/.assignment-invoked" \
    "$ADB" -s "$SERIAL" shell am instrument --no-restart -w -r \
      -e class com.TJBotmobile.t54.ParentSessionAssignmentBridgeTest#createPass54Assignment \
      -e confirmAssignmentId PASS54 \
      -e resultPath "$DEVICE_RESULT" \
      com.TJBotmobile.test/androidx.test.runner.AndroidJUnitRunner \
  2>&1 | tee "$RUNNER_LOG"

grep -Fq 'OK (1 test)' "$RUNNER_LOG"
grep -Fq 'ASSIGNMENT_CREATED route=READY result=t54-pass54-assignment.json' "$RUNNER_LOG"
if grep -Eqi 'authorization|bearer[[:space:]]|access[_ -]?token|refresh[_ -]?token' \
  "$RUNNER_LOG"; then
...
result, `jq` failure, route loss, or later strict-pass failure is terminal for Pass 54. Do not run
...
PASS54=/Users/manhhodinh/Documents/TBOT/robot/docs/evidence/t54-live-20260820-final-closeout/final-parent-sla-pass-54
HOST_RESULT="$PASS54/parent-session-bridge/t54-pass54-assignment.json"
export T54_FRESH_ASSIGNMENT_ID="$(jq -er '.assignmentId' "$HOST_RESULT")"

T54_ANSWER_LOG="$PASS54/live-capture/esp-server.log" \
T54_ANSWER_STATE_DIR="$PASS54/answer-helper-state" \
T54_ANSWER_EXPECTED_ASSIGNMENT_ID="$T54_FRESH_ASSIGNMENT_ID" \
  bash /Users/manhhodinh/Documents/TBOT/t54-answer-helper-pass9.sh \
  2>&1 | tee "$PASS54/answer-helper.log"
```
...
export T54_FRESH_ASSIGNMENT_ID="$(jq -er '.assignmentId' \
  "$PASS54/parent-session-bridge/t54-pass54-assignment.json")"
...
already-created assignment and Pass 54; never create another assignment. This handoff does not
...
- Helper and combined READY-gate output, followed by the existing strict Pass 54 evidence.
