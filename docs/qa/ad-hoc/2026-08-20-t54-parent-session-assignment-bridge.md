# T5.4 Parent-session assignment bridge: H1 Pass 53 handoff
...
`confirmAssignmentId=PASS53`, accepts no identity override, performs one HTTP `POST`, requires
...
assignment. Pass 53 adds a bounded active-root acquisition wait of at most five seconds. Both
...
HTTP, result, or helper-gate failure freezes Pass 53. After the assignment test is invoked once,
...
PASS53=/Users/manhhodinh/Documents/TBOT/robot/docs/evidence/t54-live-20260820-final-closeout/final-parent-sla-pass-53
SIGNERS="$PASS53/parent-session-bridge/signers"
...
PASS53=/Users/manhhodinh/Documents/TBOT/robot/docs/evidence/t54-live-20260820-final-closeout/final-parent-sla-pass-53
BRIDGE="$PASS53/parent-session-bridge"
PROBE_LOG="$BRIDGE/session-probe.log"
COLLECTOR="$PASS53/parent-progress"
...
JWT-shaped output freezes Pass 53 before assignment. The probe may spend at most five seconds
...
## Pass 53 one-shot pre-POST gate

The following conditions must already be true in the same immutable Pass 53 evidence window:
...
5. No assignment has been created in Pass 53 and the one-shot command below has never been
   entered.
...
PASS53=/Users/manhhodinh/Documents/TBOT/robot/docs/evidence/t54-live-20260820-final-closeout/final-parent-sla-pass-53
TIMELINE="$PASS53/live-capture/timeline.log"
COLLECTOR="$PASS53/parent-progress"
COLLISION_LOG="$PASS53/collector-logcat.txt"
: "${T54_PASS53_CAPTURE_PID:?set to the already-running Pass 53 capture PID}"

test "$T54_PASS53_CAPTURE_PID" -gt 1
kill -0 "$T54_PASS53_CAPTURE_PID"
test -s "$TIMELINE"
...
path is fixed by the Java bridge. Do not change `PASS53`, `resultPath`, the class name, or the
arming value.
...
HOST_RESULT="$BRIDGE/t54-pass53-assignment.json"
DEVICE_RESULT=/storage/emulated/0/Android/data/com.TJBotmobile/files/t54-pass53-assignment.json
...
T54_COLLECTOR_OUT="$PASS53/parent-progress" \
T54_COLLECTOR_MODE=with-uiautomation-lease \
T54_COLLECTOR_UIAUTOMATOR_COLLISIONS=0 \
  bash /Users/manhhodinh/Documents/TBOT/t54-parent-collector-pass9.sh -- \
  bash -c 'mkdir "$1"; shift; exec "$@"' _ "$BRIDGE/.assignment-invoked" \
    "$ADB" -s "$SERIAL" shell am instrument --no-restart -w -r \
      -e class com.TJBotmobile.t54.ParentSessionAssignmentBridgeTest#createPass53Assignment \
      -e confirmAssignmentId PASS53 \
      -e resultPath "$DEVICE_RESULT" \
      com.TJBotmobile.test/androidx.test.runner.AndroidJUnitRunner \
  2>&1 | tee "$RUNNER_LOG"

grep -Fq 'OK (1 test)' "$RUNNER_LOG"
grep -Fq 'ASSIGNMENT_CREATED route=READY result=t54-pass53-assignment.json' "$RUNNER_LOG"
if grep -Eqi 'authorization|bearer[[:space:]]|access[_ -]?token|refresh[_ -]?token' \
  "$RUNNER_LOG"; then
...
result, `jq` failure, route loss, or later strict-pass failure is terminal for Pass 53. Do not run
...
PASS53=/Users/manhhodinh/Documents/TBOT/robot/docs/evidence/t54-live-20260820-final-closeout/final-parent-sla-pass-53
HOST_RESULT="$PASS53/parent-session-bridge/t54-pass53-assignment.json"
export T54_FRESH_ASSIGNMENT_ID="$(jq -er '.assignmentId' "$HOST_RESULT")"

T54_ANSWER_LOG="$PASS53/live-capture/esp-server.log" \
T54_ANSWER_STATE_DIR="$PASS53/answer-helper-state" \
T54_ANSWER_EXPECTED_ASSIGNMENT_ID="$T54_FRESH_ASSIGNMENT_ID" \
  bash /Users/manhhodinh/Documents/TBOT/t54-answer-helper-pass9.sh \
  2>&1 | tee "$PASS53/answer-helper.log"
```
...
export T54_FRESH_ASSIGNMENT_ID="$(jq -er '.assignmentId' \
  "$PASS53/parent-session-bridge/t54-pass53-assignment.json")"
...
already-created assignment and Pass 53; never create another assignment. This handoff does not
...
- Helper and combined READY-gate output, followed by the existing strict Pass 53 evidence.
