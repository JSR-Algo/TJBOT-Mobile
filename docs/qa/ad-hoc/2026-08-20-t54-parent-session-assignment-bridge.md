# T5.4 Parent-session assignment bridge: H1 Pass 44 handoff
...
`confirmAssignmentId=PASS44`, accepts no identity override, performs one HTTP `POST`, requires
...
assignment. Pass 44 adds a bounded active-root acquisition wait of at most five seconds. Both
...
HTTP, result, or helper-gate failure freezes Pass 44. After the assignment test is invoked once,
...
PASS44=/Users/manhhodinh/Documents/TBOT/robot/docs/evidence/t54-live-20260820-final-closeout/final-parent-sla-pass-44
SIGNERS="$PASS44/parent-session-bridge/signers"
...
PASS44=/Users/manhhodinh/Documents/TBOT/robot/docs/evidence/t54-live-20260820-final-closeout/final-parent-sla-pass-44
BRIDGE="$PASS44/parent-session-bridge"
...
JWT-shaped output freezes Pass 44 before assignment. The probe may spend at most five seconds
waiting for a non-null active root; `ROUTE_ROOT` after that bound remains a hard pre-assignment
failure.

## Pass 44 one-shot pre-POST gate

The following conditions must already be true in the same immutable Pass 44 evidence window:
...
5. No assignment has been created in Pass 44 and the one-shot command below has never been
   entered.
...
PASS44=/Users/manhhodinh/Documents/TBOT/robot/docs/evidence/t54-live-20260820-final-closeout/final-parent-sla-pass-44
TIMELINE="$PASS44/live-capture/timeline.log"
COLLECTOR="$PASS44/parent-progress"
COLLISION_LOG="$PASS44/collector-logcat.txt"
: "${T54_PASS44_CAPTURE_PID:?set to the already-running Pass 44 capture PID}"

test "$T54_PASS44_CAPTURE_PID" -gt 1
kill -0 "$T54_PASS44_CAPTURE_PID"
test -s "$TIMELINE"
...
path is fixed by the Java bridge. Do not change `PASS44`, `resultPath`, the class name, or the
arming value.
...
HOST_RESULT="$BRIDGE/t54-pass44-assignment.json"
DEVICE_RESULT=/storage/emulated/0/Android/data/com.TJBotmobile/files/t54-pass44-assignment.json
...
# This atomic marker is written before instrumentation. It remains after every success or failure.
mkdir "$BRIDGE/.assignment-invoked"

$ADB -s "$SERIAL" shell am instrument --no-restart -w -r \
  -e class com.TJBotmobile.t54.ParentSessionAssignmentBridgeTest#createPass44Assignment \
  -e confirmAssignmentId PASS44 \
  -e resultPath "$DEVICE_RESULT" \
  com.TJBotmobile.test/androidx.test.runner.AndroidJUnitRunner \
  2>&1 | tee "$RUNNER_LOG"

grep -Fq 'OK (1 test)' "$RUNNER_LOG"
grep -Fq 'ASSIGNMENT_CREATED route=READY result=t54-pass44-assignment.json' "$RUNNER_LOG"
if grep -Eqi 'authorization|bearer[[:space:]]|access[_ -]?token|refresh[_ -]?token' \
  "$RUNNER_LOG"; then
...
result, `jq` failure, route loss, or later strict-pass failure is terminal for Pass 44. Do not run
...
PASS44=/Users/manhhodinh/Documents/TBOT/robot/docs/evidence/t54-live-20260820-final-closeout/final-parent-sla-pass-44
HOST_RESULT="$PASS44/parent-session-bridge/t54-pass44-assignment.json"
export T54_FRESH_ASSIGNMENT_ID="$(jq -er '.assignmentId' "$HOST_RESULT")"

T54_ANSWER_LOG="$PASS44/live-capture/esp-server.log" \
T54_ANSWER_STATE_DIR="$PASS44/answer-helper-state" \
T54_ANSWER_EXPECTED_ASSIGNMENT_ID="$T54_FRESH_ASSIGNMENT_ID" \
  bash /Users/manhhodinh/Documents/TBOT/t54-answer-helper-pass9.sh \
  2>&1 | tee "$PASS44/answer-helper.log"
```
...
export T54_FRESH_ASSIGNMENT_ID="$(jq -er '.assignmentId' \
  "$PASS44/parent-session-bridge/t54-pass44-assignment.json")"
...
T54_COLLECTOR_OUT="$PASS44/parent-progress" \
...
already-created assignment and Pass 44; never create another assignment. This handoff does not
...
- Helper and combined READY-gate output, followed by the existing strict Pass 44 evidence.
