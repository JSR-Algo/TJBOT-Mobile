...
- UC-A10 Logout — store action `logout`; UI trigger NOT CONFIRMED IN SOURCE CODE — promoted to active per ADR-0005 D7 + P3.B; sequence `01-identity/logout.sequence.mmd`
- UC-A11 Revoke Active Session — view other sessions (web + paired devices), revoke specific `jti`; sequence `01-identity/session-revoke.sequence.mmd`
- UC-A12 Delete Account — GDPR/COPPA right-to-erasure, 30-day grace, async pipeline; sequence `01-identity/account-delete.sequence.mmd` → `14-retention/account-deletion-pipeline.sequence.mmd`
- UC-A13 Export Account Data — GDPR data portability; sequence `01-identity/account-export.sequence.mmd`
- UC-A14 Reset Parent PIN — email magic-link recovery per ADR-0008; callable during GATE_LOCKED_OUT; sequence `01-identity/pin-recovery.sequence.mmd`
- UC-A15 Add Child Profile — per-child COPPA consent required per ADR-0011 D2 + D10; soft cap 4, hard cap 10; sequence `01-identity/child-add.sequence.mmd`
- UC-A16 Switch Active Child — client-side selector + server notify for analytics + cost-cap scoping per ADR-0011 D3; sequence `01-identity/child-switch.sequence.mmd`
- UC-A17 Suspend Child Profile — any-parent action; `children.status='archived'`; data preserved; sequence `01-identity/child-delete.sequence.mmd`
- UC-A18 Delete Child Profile — **primary-parent only per ADR-0010 D5**; 30-day retention via sys-14; sequence `01-identity/child-delete.sequence.mmd`
...
- UC-O05 Record Parental Consent (`onb_coppa` — COPPA blocker; ConsentForm SM); sequence `01-identity/coppa-consent-record.sequence.mmd`. Child profile creation gated on consent FK.
...
- UC-L22 Lesson Handoff (Parent App → Robot Start) — per ADR-0006: parent app initiates session via `POST /v1/sessions/start`; server pushes MQTT to robot; robot opens WS; parent app attaches observer lane. Sequences `04-realtime/session-start-mobile.sequence.mmd` + `04-realtime/observer-attach.sequence.mmd`.
...
### PARENT (gated)
- UC-PR01 Authenticate Parent (server-validated PIN per ADR-0005 — bcrypt, rate-limit, lockout, 24h JWT; was "numeric speed bump"); sequence `07-parent/parent-gate-validate.sequence.mmd`
...
- UC-PR08 Enable Biometric Unlock — opt-in flow per ADR-0009; FaceID / BiometricPrompt binds parent_session JWT for the 60s freshness refresh path; sequence `07-parent/freshness-refresh.sequence.mmd`
- UC-PR09 Invite Secondary Parent — primary-only per ADR-0010 D3; sequence `07-parent/household-invite-parent.sequence.mmd`
- UC-PR10 Revoke Secondary Parent — primary-only per ADR-0010 D5; revokes their parent_sessions; sequence `07-parent/household-invite-parent.sequence.mmd`
...
- UC-BU15 Cancel Order (pre-shipment); sequence TBD (P5 follow-up — gated on order_state ∈ {created, paid})
- UC-BU16 Request Refund / Return (post-fulfillment); sequence TBD (P5 follow-up)
- UC-BU17 Retry Payment (failed Stripe charge → fresh SetupIntent); sequence `19-billing/dunning-past-due.sequence.mmd` mobile re-entry path
- UC-SUB01 View Subscription Status (state + period end + payment method); sequence `19-billing/subscription-lifecycle.sequence.mmd`
- UC-SUB02 Cancel Subscription (continues until `current_period_end`; requires fresh parent JWT < 60s); sequence `19-billing/subscription-lifecycle.sequence.mmd`
- UC-SUB03 Pause Subscription (one pause per billing cycle); sequence `19-billing/subscription-lifecycle.sequence.mmd`
- UC-SUB04 Resume Subscription; sequence `19-billing/subscription-lifecycle.sequence.mmd`
- UC-SUB05 Handle Renewal Failure (past_due dunning re-entry — update payment method, retry); sequence `19-billing/dunning-past-due.sequence.mmd`
- UC-INV01 View Invoice History (read-only list of past charges); sequence TBD (P5 follow-up)
...
### MOBILE SHELL (cross-cutting OS-bridge surfaces, added P3.F / promoted P5)
- UC-MOBILE01 Handle Push Notification Deep Link (validate target, gate via parent_session if `parent_*`, redirect `pr_*` to `pr_intro`, `lesson_*` to `lesson_resume`); sequence `16-mobile/push-deep-link.sequence.mmd`. Body in `docs/usecases/domains/mobile-shell/use-cases.md`.
