...
1. Screen inventory: `SplashScreen`, `WelcomeScreen`, `IntroListenScreen`, `IntroSpeakScreen`, `IntroRetryScreen`, `IntroCelebrateScreen`, `TrustScreen`, `LoginScreen`, `ParentConsentScreen`, `ChildProfileScreen`, `MicAskScreen`, `FirstLessonEntryScreen`.
2. Primary path: `SplashScreen` -> `WelcomeScreen` -> `IntroListenScreen` -> `IntroSpeakScreen` -> `IntroRetryScreen` -> `IntroCelebrateScreen` -> `TrustScreen` -> `LoginScreen` -> `ParentConsentScreen` -> `ChildProfileScreen` -> `MicAskScreen` -> `FirstLessonEntryScreen`.
3. Edge paths: parent consent failure stays on `ParentConsentScreen` with retry; microphone permission denied routes to `MicMissingScreen`; network failure routes to `NetworkErrorScreen`; child profile save failure stays on `ChildProfileScreen` with retry; session entry failure routes to `VoiceFailedScreen`.
...
6. CTA map: "Start" -> `WelcomeScreen`; "Listen" -> `IntroListenScreen`; "Try speaking" -> `IntroSpeakScreen`; "Try again" -> `IntroRetryScreen`; "Looks good" -> `IntroCelebrateScreen`; "How TBOT stays safe" -> `TrustScreen`; auth success -> `ParentConsentScreen`; "Sign and continue" -> `/v1/auth/consent` then `ChildProfileScreen`; "Save child profile" -> `MicAskScreen`; "Allow microphone" -> OS permission then `FirstLessonEntryScreen`; "Start first lesson" -> `ConnectingScreen`; "Later" -> `HomeHubScreen`.
