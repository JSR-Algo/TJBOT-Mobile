...
- 7 external actors at `docs/usecases/actors/external/{rotjtjbot-device,realtime-voice-service,google-oauth,apple-sign-in,payment-provider,device-os,wifi-network}.md`
...
|---|---|---|
| Use-case actors (`docs/usecases/actors/*.md`) | Human-role / external-system name | `Child`, `Parent`, `Rotjtjbot Device`, `Google OAuth` |
...
| UC actor | Sequence participant(s) | Mapping notes |
|---|---|---|
| **Rotjtjbot Device** (`actors/external/rotjtjbot-device.md`) | `BLEDevice` (during pairing) → `Device` (post-claim) | `BLEDevice` is the BLE-exposed surface during sys-02 provisioning; `Device` is the cloud-bound, Wi-Fi-attached surface for all post-claim flows (sys-02..18). Same physical rotjtjbot, two participant identities to reflect the transport/lifecycle boundary. |
...
| `Device` | Rotjtjbot Device (post-claim) | sys-02..18 cloud-bound surface. |
| `BLEDevice` | Rotjtjbot Device (during pairing) | sys-02/16 boundary. |
...
| `BLEDevice` has no UC actor of its own. | Permanent: it is a lifecycle variant of Rotjtjbot Device. This glossary's §3 row is the canonical mapping. Audit AN-13 closed by this doc. |
| Payment provider identity (KD9). | Resolves when commerce ADR is written (P3.C in `.omc/plans/flow-system-immediate-fixes.md`). Update tjtjboth `payment-provider.md` and `_actors.md` once decided. |
