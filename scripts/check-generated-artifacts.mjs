...
if (!checkTrackedTree) {
  try {
    ...
    // gitnexus is optional on disposable agent worktrees without the binary.
    if (code === 'ENOENT') {
      console.warn('Generated-artifact guard: gitnexus not installed; skipping staged change analysis.');
    } else {
      const message = error instanceof Error ? error.message : String(error);
