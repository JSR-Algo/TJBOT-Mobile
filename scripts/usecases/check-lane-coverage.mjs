#!/usr/bin/env node
// AC8: parse multi-agent-execution.md lane table; verify all 15 domains covered exhaustively + disjointly (P5: mobile-shell promoted to its own folder).
...
const FILE = resolve(ROOT, 'usecases/reference/multi-agent-execution.md');
const DOMAINS_DIR = resolve(ROOT, 'usecases/domains');
...
if (fsDomains.size !== 15) {
  console.error(`FAIL fs domain count = ${fsDomains.size}, expected 15`);
  process.exit(1);
}
