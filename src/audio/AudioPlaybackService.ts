/**
 * AudioPlaybackService u2014 Streaming PCM 24kHz playback with pre-loading.
 *
 * Strategy: Buffer ~2s chunks. While one plays, pre-create the next player.
 * Audio mode is set ONCE (no switching between segments).
 * This gives near-realtime feel with smooth gapless playback.
 */
import { createAudioPlayer, setAudioModeAsync } from 'expo-audio';
import type { AudioPlayer } from 'expo-audio';

const SAMPLE_RATE = 24000;
const FIRST_BUFFER_MS = 3000; // Buffer 3s for first segment (smooth start)
const NEXT_BUFFER_MS = 3000;  // Buffer 3s for subsequent segments
const FLUSH_DELAY_MS = 400;   // Flush remaining after 400ms silence
...
export class AudioPlaybackService {
  ...
  enqueue(base64Pcm24k: string): void {
    ...
    const targetBytes = this.isFirstSegment
      ? SAMPLE_RATE * 2 * (FIRST_BUFFER_MS / 1000)
      : SAMPLE_RATE * 2 * (NEXT_BUFFER_MS / 1000);

    if (this.totalSize >= targetBytes) {
      this._flushToReady();
    }

    // Reset flush timer for remaining data
