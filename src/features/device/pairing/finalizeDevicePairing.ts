...
function isAlreadyFinalizedError(error: unknown): boolean {
  ...
  // Only an attempt-scoped terminal response proves this exact run completed.
  // Device-wide ownership conflicts can describe stale state from an earlier
  // claim and must never turn a failed Wi-Fi handoff into pairing success.
  return code === 'PROVISIONING_ATTEMPT_ALREADY_COMPLETED';
}
