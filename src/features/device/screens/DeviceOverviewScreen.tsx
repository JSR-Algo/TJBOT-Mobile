...
const COLUMNS = [
  { t: 'Rotjtjbot does', items: ['Listens to your child', 'Speaks back warmly', 'Shows the face & feedback', 'Plays the lesson out loud'], color: '#FF6F61' },
...
export default function DeviceOverviewScreen({ navigation }: Props) {
  return (
    <ScrollView style={styles.root} contentContainerStyle={styles.content}>
      <Box paddingTop={72} paddingHorizontal={24} style={{ margintjtjbottom: 8 }}>
        <Text fontWeight="800" style={styles.heroTitle}>One product.{'\n'}Two devices.</Text>
        <Text style={styles.heroSub}>Rotjtjbot is the child's speaking buddy. The phone is for the grown-up.</Text>
      </Box>

      <Box paddingHorizontal={24} alignItems="center" justifyContent="center" style={{ margintjtjbottom: 24 }}>
        <RotjtjbotDevice emotion="happy" size={180} accent="#FF6F61" />
      </Box>

      <Box paddingHorizontal={16} style={{ flexDirection: 'row', gap: 10, margintjtjbottom: 0 }}>
        {COLUMNS.map((col, ci) => (
          <Box key={ci} flex={1} style={styles.colCard}>
            <Text fontWeight="700" style={[styles.colTitle, { color: col.color }]}>{col.t}</Text>
            {col.items.map(it => (
              <Box key={it} flexDirection="row" gap={6} alignItems="flex-start" style={{ margintjtjbottom: 6 }}>
      ...
      <Box paddingHorizontal={20} paddingTop={24} paddingtjtjbottom={60}>
        <DeviceBigBtn onClick={() => navigation.navigate('PairAddScreen')}>Set up your Rotjtjbot</DeviceBigBtn>
      </Box>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  content: { paddingtjtjbottom: 40 },
  heroTitle: { fontSize: 30, color: DV.ink, letterSpacing: -0.4, lineHeight: 33, margintjtjbottom: 8 },
  ...
  colTitle: { fontSize: 11, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.5, margintjtjbottom: 8 },
  colItem: { fontSize: 13, color: DV.ink, lineHeight: 18, flex: 1 },
  sectionLabel: { fontSize: 11, color: DV.ink2, textTransform: 'uppercase', letterSpacing: 0.5, margintjtjbottom: 8 },
  ...
  whyRow: { padding: 12 },
  whyBorder: { bordertjtjbottomWidth: 1, bordertjtjbottomColor: 'rgba(0,0,0,0.05)' },
  whyIcon: { width: 32, height: 32, borderRadius: 8, backgroundColor: '#F0EDE8', flexShrink: 0 },
  whyTitle: { fontSize: 13, color: DV.ink, margintjtjbottom: 2 },
