...
export default function LessonSummaryScreen({ navigation, route }: Props): React.JSX.Element {
  ...
  const reward = inbox.data?.rewards.find(item => item.child.id === route.params?.childId && item.robot.id === route.params?.deviceId);
  return (
