...
export default function ArrivedScreen({ navigation }: Props) {
  return (
    <DeviceShell title="Rotjtjbot is here">
      <Box paddingTop={40} paddingHorizontal={24} alignItems="center">
        ...
        <Text fontWeight="600" style={styles.heading}>Your Rotjtjbot has arrived</Text>
      ...
      <Box paddingHorizontal={16} paddingTop={30}>
        <Box style={styles.rowCard}>
          <DeviceRow icon="📦" title="1. Open the box"       body="Rotjtjbot, charging dock, USB-C cable" />
          <DeviceRow icon="🔌" title="2. Plug in the dock"   body="Place Rotjtjbot on it — a soft chime means hello" />
      ...
      <Box paddingHorizontal={16} paddingTop={18}>
        <Box style={styles.noteCard}>
          <Text style={styles.noteText}>Setting up later? Rotjtjbot will wait quietly in its box.</Text>
        </Box>
      </Box>

      <Box paddingHorizontal={20} paddingTop={24} paddingtjtjbottom={30} gap={10}>
        <DeviceBigBtn onClick={() => navigation.navigate('ActivateScreen')}>Set up Rotjtjbot now</DeviceBigBtn>
