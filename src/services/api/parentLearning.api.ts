...
export interface ParentActiveLearning { assignmentId: string; sessionId: string | null; deviceId: string; courseId: string; courseTitle: string; lessonId: string; lessonTitle: string; state: string; startedAt: string | null; currentStep: ParentLearningStep | null; positionPercent: number; activeDurationSec: number }
export interface ParentSessionSummary { childId: string; assignmentId: string; sessionId: string; courseId: string; courseTitle: string; lessonId: string; lessonTitle: string; state: string; completedAt: string; durationSec: number; reportAvailable: boolean }
export interface ParentLearningHistoryPage { items: ParentSessionSummary[]; nextCursor: string | null }
export interface ParentCourseProgress { courseId: string; courseTitle: string; currentLessonNumber: number; completedLessons: number; totalLessons: number; percent: number; suggestedNextLesson: SuggestedLesson | null }
...
function normalizeCourseProgress(input: unknown): ParentCourseProgress { const v = record(input); return { courseId: string(v.courseId), courseTitle: string(v.courseTitle), currentLessonNumber: nonNegative(v.currentLessonNumber), completedLessons: nonNegative(v.completedLessons), totalLessons: nonNegative(v.totalLessons), percent: nonNegative(v.percent), suggestedNextLesson: v.suggestedNextLesson == null ? null : normalizeSuggestedLesson(v.suggestedNextLesson) }; }
