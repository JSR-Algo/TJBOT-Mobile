...
User flows and screen navigation are split across 12 src domains and assigned to 4 agent lanes (A/B/C/D) plus Lane Z (infra + integration). The single canonical store is `nav-graph-data.json`; per-domain mermaid + go-call artefacts under `docs/flows/domains/<d>/` regenerate deterministically via `npm run flows:fast`.

See `docs/flows/AGENTS.md` for the lane ownership matrix and single-writer process discipline for `nav-graph-data.json`. See `docs/decisions/0004-flows-modular-restructure.md` for the design decisions and rejected alternatives.
