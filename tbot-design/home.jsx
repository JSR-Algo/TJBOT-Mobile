  ...
  return (
    <ScreenShell bg={bg}>
      ...
      <div style={{ position:'absolute', left:24, right:24, bottom:36, display:'flex', gap:10 }}>
        <HomeSecondaryBtn label="Course"   icon="🗺️" onClick={()=>go('course')}           dim={cfg.dimSecondary}/>
        <HomeSecondaryBtn label="Review"   icon="🔁" onClick={()=>go('review_entry')}    dim={cfg.dimSecondary} badge={cfg.reviewBadge}/>
        <HomeSecondaryBtn label="Progress" icon="⭐" onClick={()=>go('today_progress')}  dim={cfg.dimSecondary}/>
      </div>
