  ...
  return (
    <DvShell title="Add Yummy Words" onBack={()=>go('cl_detail')}>
      ...
      <div style={{ padding:'20px 16px 0' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
          {[
            { id:'library', tag:'Best for families', title:'All Courses', body:'Every course, now and future, on your Robot.', priceUsd:8.99, period:'/ month', cancel:'Cancel anytime · 7-day free trial' },
            { id:'single', tag:'One-time', title:'Just this course', body:'Yummy Words only, yours forever.', priceUsd:24, period:'one-time', cancel:'No subscription' },
          ].map(p=>{
            const sel = plan===p.id;
            return (
              <button key={p.id} onClick={(e)=>{e.stopPropagation(); setPlan(p.id);}} style={{
                ...
                <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:10 }}>
                  ...
                  <div style={{ textAlign:'right' }}>
                    <div style={{ fontSize:18, fontWeight:700, color:CL.ink }}>{(window.__tbot && window.__tbot.fmtPrice) ? window.__tbot.fmtPrice(p.priceUsd, 'USD') : `$${p.priceUsd}`}</div>
