...
function ParentScroll({ children, title, onBack, right, persona='parent' }){
  return (
    <div data-persona={persona} style={{ height:'100%', overflow:'auto', background:PA.bg, position:'relative', WebkitFontSmoothing:'antialiased', fontFamily:'-apple-system, BlinkMacSystemFont, "Inter", "SF Pro Text", system-ui, sans-serif', color:PA.ink }}>
      {title !== undefined && (
        <div style={{
...
function S_ParentHistory({ go }){
  ...
  // group by week
  const fmtDay = d => (window.__tbot?.fmtDate ? window.__tbot.fmtDate(d, { weekday:'short', month:'short', day:'numeric' }) : d.toLocaleDateString('en-US', { weekday:'short', month:'short', day:'numeric' }));
            ...
            <div key={d.i} style={{
              ...
              <div style={{
                ...
                <div style={{ fontSize:10, color:PA.ink3, textTransform:'uppercase', letterSpacing:0.5, fontWeight:600 }}>
                  {window.__tbot?.fmtDate ? window.__tbot.fmtDate(d.date, { month:'short' }) : d.date.toLocaleDateString('en-US',{ month:'short' })}
                </div>
