...
const ACTORS_PATH = path.join(PROJECT_ROOT, 'docs', 'sequences', '_actors.md');
...
// ─── Parse DBML ─────────────────────────────────────────────────────────────
// Minimal DBML parser: extract enum blocks, Table blocks (with columns), Ref: lines, and Indexes blocks.
function parseDbml(src, absPath) {
  const file = rel(absPath);
  const lines = src.split(/\r?\n/);
  const tables = [];          // {name, line, columns:[{name,type,attrs,line}], indexes:[{cols,line,unique,note}], block: [startLine,endLine]}
  const refs = [];            // {leftTable,leftCol,rightTable,rightCol,line}
  ...
  while (i < lines.length) {
    ...
    if (enumM) {
      enums.push({ name: enumM[1], line: lineNo });
    ...
    if (tableM) {
      const tbl = { name: tableM[1], line: lineNo, columns: [], indexes: [], block: [lineNo, lineNo] };
      i++;
      while (i < lines.length) {
        const inner = lines[i];
        const innerLine = i + 1;
          ...
          while (i < lines.length && !lines[i].includes('}')) {
            ...
            if (idxTrim) {
              ...
              if (m) {
                ...
                tbl.indexes.push({ cols, unique, note, line: idxLineNo });
              }
            }
            i++;
          }
  ...
  for (let li = 0; li < lines.length; li++) {
    ...
    // numeric / decimal / float — only forbidden when annotated as money OR named like *_cents/*_amount.
