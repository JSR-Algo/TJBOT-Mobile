...
const PUML_DIR = resolve(ROOT, 'docs/architecture/usecases');
const OUT_INDEX = resolve(ROOT, 'docs/usecases/reference/use-case-index.json');
const OUT_OVR = resolve(ROOT, 'docs/usecases/reference/alias-overrides.json');
...
legacy.forEach((line, i) => {
  ...
  const u = line.match(/^- (UC-[A-Z]{1,2}\d+)\s+(.+)$/);
...
for (const e of entries) {
  // Special case: device-mgmt UCs have no puml
  if (e.domain === 'device-mgmt') {
    e.aliases = [];
    overrides.push({ id: e.id, override: 'no-puml', reason: 'KD5 — Lane D adds device-mgmt.usecase.puml in Phase 1' });
    continue;
  }
