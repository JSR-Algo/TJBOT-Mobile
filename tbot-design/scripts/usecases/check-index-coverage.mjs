#!/usr/bin/env node
// AC1 (count=129) + AC11 (subset of legacy) + AC14 (alias completeness ≥95% of (129−no_puml)).
...
const INDEX = resolve(ROOT, 'docs/usecases/reference/use-case-index.json');
...
// AC1: count == 129
if (idx.entries.length !== 129)
  fail(`AC1 count=${idx.entries.length}, expected 129`);
ok(`AC1 count=129`);

// AC11: keys subset of legacy
const legacyIds = new Set();
for (const line of readFileSync(LEGACY, 'utf8').split('\n')) {
  const m = line.match(/^- (UC-[A-Z]{1,2}\d+)\b/);
...
// AC14: alias completeness ≥ 95% of (129 − no_puml)
