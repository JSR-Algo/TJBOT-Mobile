...
export default function TBPhone({ children, bg = 'var(--tb-cream)', dark = false, status = true, indicator = true, statusDark = false }) {
  return (
...
export function TBBigBtn({ children, color = 'var(--tb-coral)', textColor = '#fff', onClick, style, icon }) {
  return (
...
export function TBMicButton({ state = 'idle', size = 130, onClick }) {
...
export function TBBubble({ children, color = '#fff', textColor = 'var(--tb-ink)', tail = 'bl', style }) {
  return (
    <div style={{
      background: color, color: textColor,
...
// Lesson card
export function TBLessonCard({ title, subtitle, accent = 'var(--tb-coral)', emoji = '👋', state = 'available', progress = 0, onClick }) {
...
// Confetti — pure CSS
export function TBConfetti({ count = 24 }) {
...
// Sparkle layer
export function TBSparkles({ count = 8 }) {
  return (
...
// Avatar tile
export function TBAvatar({ kind = 'fox', selected = false, onClick, size = 84 }) {
  const map = {
    fox: { bg: '#F2A65A', emoji: '🦊' },
...
// Sticker badge
export function TBSticker({ icon = '⭐', label, color = 'var(--tb-yellow)', size = 88, locked = false }) {
  return (
...
export function TBTopbar({ title, onBack, right, style }) {
  return (
    <div style={{
      display: 'flex', alignItems: 'center', justifyContent: 'space-between',
...
// Generic icon
export function TBIcon({ name, size = 22, color = 'currentColor' }) {
  const paths = {
...
export function TBParentTabBar({ active = 'home' }) {
  const tabs = [
