  ...
  React.useLayoutEffect(() => {
    ...
    try {
      ...
      if (s && Number.isFinite(s.x) && Number.isFinite(s.y) && Number.isFinite(s.scale)) {
        ...
        const vh = vp ? vp.getBoundingClientRect().height : 0;
        const vw = vp ? vp.getBoundingClientRect().width  : 0;
        // If saved transform would push content entirely below/right of the
        // visible viewport (small preview pane, stale state, etc.), reset.
        const offscreen = (vh && s.y > vh - 100) || (vw && s.x > vw - 100);
        if (!offscreen) {
...
export { DCSection, DCArtboard, DCPostIt };
