...
1. **Happy path** — child opens app, logs in, enters RotjtjbotScreen, completes a 3-turn English conversation, disconnects cleanly.
...
3. **COPPA gate** — a child without consent is blocked from entering RotjtjbotScreen.
4. **Feature flag off** — a build without `EXPO_PUBLIC_V1_ROtjtjbot` routes home to legacy `InteractionScreen`; RotjtjbotScreen is absent.
5. **Runtime kill switch** — with flag on but backend `v1_rotjtjbot_enabled_remote=false`, RotjtjbotScreen entry is disabled.
...
```
GIVEN the app is built with EXPO_PUBLIC_V1_ROtjtjbot=true
...
THEN RotjtjbotScreen loads within 2 s
  AND the rotjtjbot avatar is in IDLE_BREATHING expression
WHEN the child taps-and-holds, says "hi"
THEN within 2.5 s first audio chunk arrives (instrumented; E2E only asserts ≤ 5 s)
  AND the rotjtjbot transitions IDLE → LISTENING → THINKING → SPEAKING
...
THEN RotjtjbotScreen is NOT entered
...
```
GIVEN the app is rebuilt with EXPO_PUBLIC_V1_ROtjtjbot unset
WHEN parent logs in and taps the home tab
THEN legacy InteractionScreen appears
  AND navigation does NOT include a route named "RotjtjbotScreen"
...
```
GIVEN app built with EXPO_PUBLIC_V1_ROtjtjbot=true
  AND backend config has v1_rotjtjbot_enabled_remote=false
WHEN parent opens the app (cold start)
THEN /v1/config returns v1_rotjtjbot_enabled_remote=false
...
- All S-01..S-06 pass on tjtjboth P0 platforms in a single CI run.
