...
describe('main tab active state', () => {
  ...
  it('keeps the canonical Hello Maestro header and Devices entry in the shared shell', () => {
    const navigation = {
    ...
    expect(screen.getByText('Hello Maestro')).toBeTruthy();
    expect(screen.queryByText('Today')).toBeNull();
    expect(StyleSheet.flatten(screen.getByTestId('appShellHomeTitleBlock').props.style)).toEqual(
      expect.objectContaining({ flex: 1, minWidth: 0, overflow: 'hidden' }),
    );
  ...
  it('falls back to Home when an OTA receives a stale persisted route', () => {
    const staleRoute = `Legacy${ROUTES.HomeChildProfileScreen}` as keyof RootStackParamList;
